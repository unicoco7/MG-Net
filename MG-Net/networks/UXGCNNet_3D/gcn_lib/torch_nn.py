...
class BasicConv3D(Seq):
    def __init__(self, channels, act='relu', norm=None, bias=True, drop=0., kernel_size=1, padding=0, groups=4):
        m = []
        for i in range(1, len(channels)):
            # Modify Conv2d to Conv3d
            m.append(Conv3d(channels[i - 1], channels[i], kernel_size, padding=padding, bias=bias, groups=groups))        
            # Modify norm_layer to handle 3D case
            if norm is not None and norm.lower() != 'none':
                m.append(norm_layer(norm, channels[-1]))            
            # Modify act_layer to handle 3D case
            if act is not None and act.lower() != 'none':
                m.append(act_layer(act))           
            if drop > 0:
